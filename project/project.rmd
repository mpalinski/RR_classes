...
simulations <-
  data.frame(date = c(first(last_dates, 29), seq(
    last(last_dates, 1), by = "day", length.out = 30
  )))

for (i in 1:sims_num) {
  simulated <- ugarchsim(garch_model, n.sim = 29)
  simulations[, ncol(simulations) + 1] <- c(last_values, simulated@simulation$seriesSim)
  colnames(simulations)[ncol(simulations)] <- paste0("Path", i)
}

simulations <-
  melt(data.table(simulations),  id.vars = 'date', variable.name = 'series')

ggplot(simulations, aes(date, value)) + geom_line(aes(colour = series)) +
  theme_modern_rc() +
  geom_vline(
    xintercept = last(last_dates, 1),
    color = "white",
    linetype = "dashed",
    size = 1
  ) +
