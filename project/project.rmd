...
params:
  ...
  Columns: 
    value: "None"
    label: "Which prices should be included? (remove with DEL)"
...
- SimulationPaths: ``r params$SimulationPaths`` days
...
Below you can find a handful of descriptive statistics regarding the columns specified during knitting. Maximum price recorded for ``r params$Company`` during ``r params$From`` - ``r params$To`` period was \$`r round(max(descriptive_no_volume[,-1]),2)`, while the minimum was \$`r round(min(descriptive_no_volume[,-1]),2)`. 
...
for (i in 1:length(names(quotes_xts))) {
  cat("###", names(quotes_xts)[i], " \n")
  ...
  p <- ggplot(return, aes(x = daily.returns)) +
    ...
    scale_x_continuous(breaks = scales::pretty_breaks(n = 5),
                       expand = c(0, 0)) +
    labs(y = NULL, x = paste("Log return of", names(quotes_xts)[i])) +
...
fit2 = Arima(return, order = c(4,0,4), include.drift = F)

last_return <- last(return, 1)
last_date <- index(last_return)
last_value <- coredata(last_return)

sims_num <- params$SimulationPaths

df <- data.frame(date=seq(last_date, by="day", length.out=32))

for(i in 1:sims_num) {
  df[ , ncol(df) + 1] <- c(last_value, simulate(fit2, nsim=31))
  colnames(df)[ncol(df)] <- paste0("Path", i)
}

df <- melt(data.table(df),  id.vars = 'date', variable.name = 'series')
