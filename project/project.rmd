...
- Forecast: `r params$Forecast` days
...
This chapter provides insight on the log returns of selected ([info](https://quantivity.wordpress.com/2011/02/21/why-log-returns/) on why log returns instead of simple returns is used in finance).
...
for (i in 1:length(names(quotes_xts))) {
  ...
  p <- ggplot(return, aes(x = daily.returns)) +
    geom_histogram(color = "white", bins = 30, aes(y=..density..)) +
    geom_line(aes(y = ..density..), color = "green", size = 1.2, alpha = 0.8, stat = 'density') +  
    theme_modern_rc() +
